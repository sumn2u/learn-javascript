...
语句在口语中（也是更常见的说法）被称为一 _行代码_。这是因为语句通常写在单独的行上。因此，程序是自上而下、从左到右阅读的。你也许会好奇，什么是代码（也称源代码）。这是一个很宽泛的术语，既可以指整个程序，也可以指最小的部分。因此，一行代码就是你程序中的一行。
...
这段代码可以由另一个称为 _解释器_ 的程序来执行，它会读取代码并以正确的顺序执行所有语句。
